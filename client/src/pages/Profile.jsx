    ...
    <div className="page">
      <div className="profile-header">
        ...
        <div>
          ...
          {!editingBio ? (
            <p>{profile.bio || (isOwnProfile ? 'Add a short bio to tell people about yourself.' : '')}</p>
          ) : null}
        </div>
        {isOwnProfile && !editingBio && (
          <button className="btn btn-ghost btn-sm" style={{ marginLeft: 'auto' }} onClick={() => setEditingBio(true)}>
            Edit bio
          </button>
        )}
      </div>
      ...
      {editingBio && (
        <div className="form-group" style={{ maxWidth: 480, marginBottom: 24 }}>
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 8 }}>
            ...
            <button className="btn btn-ghost btn-sm" onClick={() => { setEditingBio(false); setBioDraft(profile.bio || ''); }}>
              Cancel
            </button>
          </div>
        </div>
